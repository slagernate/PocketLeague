    ...
    override func update(currentTime: CFTimeInterval) {
		...
		let updateNode: (SKNode!, UnsafeMutablePointer<ObjCBool>) -> Void = {
			...
			if (self.car1.steerLeft) {
				self.car1.physicsBody?.applyTorque(CGFloat(10))
			} else if (self.car1.steerRight) {
				self.car1.physicsBody?.applyTorque(CGFloat(-10))
			}
			print(self.car1.position)
			
			//car.physicsBody?.velocity = CGVectorMake(CGFloat(cos(self.zRotation)) * car.speed, CGFloat(sin(self.zRotation)) * car.speed)
			...
			if (self.car1.accelerate) {
    			if (carVelocity < 60) {
        			self.car1.physicsBody?.applyImpulse(CGVectorMake(CGFloat(cos(self.zRotation)) * self.car1.speed, CGFloat(sin(self.zRotation)) * self.car1.speed))
    			}
    		}
		}
